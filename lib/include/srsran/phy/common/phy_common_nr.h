...
/**
 * @brief Defines the number of subcarriers per resource block. Defined by TS 38.101.
 */
#define NRSCOPE_NSC_PER_RB_NR 12U
...
typedef struct {
  ...
  uint32_t nof_ss;
  float M;
  uint32_t first_symbol_idx; ///< Defined by TS 36.213 section 13 UE procedure for monitoring Type0-PDCCH CSS sets:
} coreset_zero_t_f_entry_nrscope;

/**
 * @brief Check the table 13-11 from 38.213
 * @param entry_idx Entry index of the table, from mib.ss0_idx
 * @param ssb_idx SSB index, or i in table 13-11, from mib.ssb_idx
 * @param coreset_symb coreset zero's duration
 * @param cfg the parameter from the table, including O, M, nof_ss, first symbol index
 * @return 0 for successfully checking the table
 */
int coreset_zero_t_f_nrscope(uint32_t entry_idx, uint32_t ssb_idx, uint32_t coreset_symb, coreset_zero_t_f_entry_nrscope* cfg);
