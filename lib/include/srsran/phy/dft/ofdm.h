...
/**
 * @brief Initialises or reconfigures OFDM receiver for 5G NR, added in nrscope by Haoran Wan
 *
 * @note The reconfiguration of the OFDM object considers only CP, number of PRB and optionally the FFT size
 * @attention The OFDM object must be zeroed externally prior calling the initialization for first time
 *
 ...
 * @return SRSRAN_SUCCESS if the initialization/reconfiguration is successful, SRSRAN_ERROR code otherwise
 */
SRSRAN_API int srsran_ofdm_rx_init_cfg_nrscope(srsran_ofdm_t* q, srsran_ofdm_cfg_t* cfg, int scs_idx);
...
SRSRAN_API int srsran_ofdm_set_phase_compensation_nrscope(srsran_ofdm_t* q, double center_freq_hz, int scs_idx, uint32_t nof_prbs);
