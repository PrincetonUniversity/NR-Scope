...
{
  // commented by Haoran, this is the same as srsRAN_Project code
  // if (nof_time_res == 0) {
...
static int dci_nr_format_0_1_unpack(const srsran_dci_nr_t* q, srsran_dci_msg_nr_t* msg, srsran_dci_ul_nr_t* dci)
{
  ...
  // Precoding information and number of layers
  if (cfg->pusch_tx_config_non_codebook) {
    ERROR("Not implemented for precoding information");
    // return 0;
  }
...
static int dci_nr_format_1_1_unpack(const srsran_dci_nr_t* q, srsran_dci_msg_nr_t* msg, srsran_dci_dl_nr_t* dci)
{
  ...
  if (msg->nof_bits != nof_bits) {
    ...
    printf("[ue activity error skip] Invalid number of bits %d, expected %d\n", msg->nof_bits, nof_bits);
    // return SRSRAN_ERROR;
  }
  ...
  if (nof_unpacked_bits != nof_bits) {
    ...
    printf("[ue activity error skip] Unpacked bits read (%d) do NOT match payload size (%d)\n", nof_unpacked_bits, nof_bits);
    // return SRSRAN_ERROR;
  }
