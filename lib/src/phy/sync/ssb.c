...
static int ssb_setup_corr(srsran_ssb_t* q)
{
  ...
  for (uint32_t N_id_2 = 0; N_id_2 < SRSRAN_NOF_NID_2_NR; N_id_2++) {
    ...
    // printf("q->corr_sz: %d\n", q->corr_sz);
    // FILE *fp;
    // fp = fopen("generated_pss.txt", "a");
    // fwrite(q->tmp_freq, sizeof(cf_t), q->corr_sz, fp);
    // fclose(fp);
...
int srsran_ssb_set_cfg(srsran_ssb_t* q, const srsran_ssb_cfg_t* cfg)
{
  ...
  // printf("calculated offset: %f\n", (double)q->f_offset * q->scs_hz);
  // printf("central frequency offset: %f\n", freq_offset_hz);
  // printf("scs: %f\n", q->scs_hz);
  // printf("nof scs: %d\n", q->f_offset);
                          ...
                          cf_t          ssb_grid[SRSRAN_SSB_NOF_RE])
{
  ...
  for (uint32_t l = 0; l < SRSRAN_SSB_DURATION_NSYMB; l++) {
    ...
    // printf("q->cp_sz: %u\n", q->cp_sz);
    // printf("q->cfg.center_freq_hz: %f\n", q->cfg.center_freq_hz);
                          ...
                          float*        coarse_cfo_hz)
{
  ...
  while ((t_offset + q->symbol_sz) < nof_samples) {
    ...
    // printf("shifted numbers: %d, total numbers: %d\n", t_offset, n);
    ...
    for (uint32_t N_id_2 = 0; N_id_2 < SRSRAN_NOF_NID_2_NR; N_id_2++) {
      ...
      //   FILE *fp;
      //   fp = fopen("seq0.txt", "w");
      //   fwrite(q->pss_seq[N_id_2], sizeof(cf_t), q->corr_sz, fp);
      //   fclose(fp);
      // }else if(N_id_2 == 1){
      //   FILE *fp;
      //   fp = fopen("seq1.txt", "w");
  ...
  // From the best sequence correlate in frequency domain
  {
    ...
    // if(best_N_id_2 == 2){
    //   // printf("save pss result, corr_sz: %d.\n", q->corr_sz);
    //   ssb_vec_prod_conj_circ_shift(q->tmp_freq, q->pss_seq[best_N_id_2], q->tmp_corr, q->corr_sz, best_shift);
    //   FILE *fp;
    //   fp = fopen("pss_correlate_result.txt", "a");
    //   fwrite(q->tmp_corr, sizeof(cf_t), q->corr_sz, fp);
    //   fclose(fp);
    //   FILE *fp_in;
    //   fp_in = fopen("in_data.txt", "a");
    //   fwrite(q->tmp_freq, sizeof(cf_t), q->corr_sz, fp_in);
    //   fclose(fp_in);
    // }
  }
...
int srsran_ssb_search(srsran_ssb_t* q, const cf_t* in, uint32_t nof_samples, srsran_ssb_search_res_t* res)
{
  ...
  // if(q->cfg.ssb_freq_hz == 2524950000){
  // FILE *fp;
  // fp = fopen("output_0603_searcher_sa.txt", "a");
  // fwrite(in, sizeof(cf_t), nof_samples, fp);
  // fclose(fp);
  // printf("nof_samples in srsran_ssb_search: %d\n", nof_samples);
  // }

  // if(q->cfg.ssb_freq_hz == 2528550000){
  //   FILE *fp;
  //   fp = fopen("output_252855.txt", "a");
  //   fwrite(in, sizeof(cf_t), nof_samples, fp);
  //   fclose(fp);
  //   printf("nof_samples in srsran_ssb_search: %d\n", nof_samples);
  // }
  ...
  // if(){
  //   FILE *fp;
  //   fp = fopen("in_data_929.txt", "a");
  //   fwrite(in, sizeof(cf_t), nof_samples, fp);
  //   fclose(fp);
    
  //   printf("N_id: %d\n", N_id);
  //   printf("ssb.c, pbch_meas.corr: %f\n", pbch_meas.corr);
  //   printf("ssb.c, q->args.pbch_dmrs_thr: %f\n", q->args.pbch_dmrs_thr);
  // }
  // printf("ssb search, pbch_meas.corr: %f\n", pbch_meas.corr);
  // printf("ssb.c, q->args.pbch_dmrs_thr: %f\n", q->args.pbch_dmrs_thr);
  ...
  // printf("MIB, coreset0_idx: %d\n", mib.coreset0_idx);
  // printf("MIB, scs_common: %d\n", mib.scs_common);
                    ...
                    srsran_pbch_msg_nr_t*          pbch_msg)
{
  ...
  // Avoid decoding if the selected PBCH DMRS do not reach the minimum threshold
  if (pbch_meas.corr < q->args.pbch_dmrs_thr) {
    // printf("ssb_find pbch_meas.corr: %f\n", pbch_meas.corr);
    return SRSRAN_SUCCESS;
  }
  // printf("success pbch_meas, pbch_meas.corr: %f\n", pbch_meas.corr);
