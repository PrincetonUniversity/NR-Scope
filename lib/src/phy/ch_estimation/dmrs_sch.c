                             ...
                             srsran_chest_dl_res_t*       chest_res)
{
  ...
  // FILE *fp;
  // fp = fopen("DMRS_252495.txt", "a");
  // printf("corr length: %d\n", nof_symbols);
  // fwrite(&corr[0], sizeof(cf_t), nof_symbols, fp);
  // fclose(fp);
  ...
  for (uint32_t i = 0; i < nof_symbols - 1; i++) {
    float time_diff  = srsran_symbol_distance_s(symbols[i], symbols[i + 1], q->carrier.scs); // actually has some problem here 
  ...
  if (dmrs_cfg->type == srsran_dmrs_sch_type_1) {
    ...
    if (srsran_interp_linear_resize(&q->interpolator_type1, nof_pilots_x_symbol, 2) < SRSRAN_SUCCESS) {
      ERROR("[Interpolator] Resizing interpolator nof_pilots_x_symbol=%d; M=%d;\n", nof_pilots_x_symbol, 2);
      return SRSRAN_ERROR;
    }
    // printf("Resizing interpolator nof_pilots_x_symbol=%d; M=%d;", nof_pilots_x_symbol, 2);
    // Interpolate
  ...
  uint32_t count = 0;
  // printf("[Interpolator] grant->S: %u\n", grant->S);
  // printf("[Interpolator] grant->L: %u\n", grant->L);
  for (uint32_t l = grant->S; l < grant->S + grant->L; l++) {
    ...
    for (uint32_t i = 0; i < nof_re_x_symbol; i++) {
      if (!rvd_mask[i]) {
#if DMRS_SCH_CFO_PRECOMPENSATE
        chest_res->ce[0][0][count++] = ce[i]* cfo_correction[l]; // disable cfo correlation for debugging
        // printf("cfo_correction[l]: %f+%fi\n", creal(chest_res->ce[0][0][count-1]), cimag(chest_res->ce[0][0][count-1]));
...
int srsran_dmrs_sch_estimate_nrscope(srsran_dmrs_sch_t*           q,
                             const srsran_slot_cfg_t*     slot,
                             const srsran_sch_cfg_nr_t*   cfg,
                             const srsran_sch_grant_nr_t* grant,
                             const cf_t*                  sf_symbols,
                             srsran_chest_dl_res_t*       chest_res,
                             double *                     phase_diff_f_s
                             )
{
  const uint32_t delta = 0;
  ...
  //added by Haoran, to compensate the 15kHz SCS caused phase shift
  double phase_pilot[3];
  for (int i = 0; i < nof_symbols; i++){
    phase_pilot[i] = atan2(cimag(corr[i]), creal(corr[i]));
    // printf("phase_pilot[i]: %f\n", phase_pilot[i]);
  }
  if(fabs(phase_pilot[1] - phase_pilot[0]) > 10 * fabs(phase_pilot[2] - phase_pilot[1])){
    // printf("over threshold\n");
    if(*phase_diff_f_s == 0){
      // printf("set value\n");
      *phase_diff_f_s = (phase_pilot[1] - phase_pilot[0]); // setup the phase diff for future compensation
    }
  }
