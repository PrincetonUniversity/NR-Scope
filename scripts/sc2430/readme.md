...
The dci decoder optimizations are essentially memoization. Previously, the decoder iterated over the slot separately for each RNTI, decoding the locations where the RNTI's message may appear. This duplicates work and adds cache pressure. The optimized decoders pre-compute all the possible locations for DCIs, perform most of the common pipeline once for each candidate location, and then perform a small amount of RNTI-specific work. This also means the optimized decoder is much less sensitive to workload, so we anticipate more significant performance improvements in higher volume scenarios.